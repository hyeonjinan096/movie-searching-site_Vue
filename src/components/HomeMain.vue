    ...
    <div class="flex flex-wrap w-full gap-4 p-4 bg-gray-900">
      <ul class="flex flex-wrap w-full mx-10 justify-center items-center">
        <li
          ...
          <RouterLink
            :to="{ name: 'Detail', params: { imdbiId: movie.imdbID } }">
            <div
              class="item relative w-full h-full overflow-hidden rounded-lg transition hover:scale-90">
              <img
                :src="movie.Poster"
                alt=""
                class="w-full h-full object-cover" />
              <div
                class="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center transition duration-300 opacity-0 hover:opacity-100">
                <h2
